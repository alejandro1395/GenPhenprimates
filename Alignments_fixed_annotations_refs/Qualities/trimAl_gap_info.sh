...
${BIN} -in ${filepath} \
-sgt > ${OUTDIR}Prot_alignments_qual/${species_name}/${species_name}.${gene_name}.SpeciesGaps" > ${OUTDIR}Prot_alignments_qual/qu/${species_name}.${gene_name}.trimAl_spc_gap.sh
jobname=$(echo ${OUTDIR}Prot_alignments_qual/qu/${species_name}.${gene_name}.trimAl_spc_gap.sh)
chmod 755 $jobname

#SUBMISSION TO CLUSTER
#/scratch/devel/avalenzu/CNAG_interface/submit.py -c ${jobname} -o ${OUTDIR}Prot_alignments_qual/out/${species_name}.${gene_name}.trimAl_spc_gap.out \
#-e ${OUTDIR}Prot_alignments_qual/out/${species_name}.${gene_name}.trimAl_spc_gap.err -n ${species_name}.${gene_name} -u 1 -t 1 -w 00:05:00
done
