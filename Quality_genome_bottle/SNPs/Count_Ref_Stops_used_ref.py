...
#DICT SPECIES TAGS
Species_tags = pd.read_csv(reseq_used, sep='\t', low_memory=False)#panda creation
...
for record in alignment_obj:
    ...
    if "PD" not in curr:
        ...
        for ind in individuals_ids:
            if curr.endswith(str(ind)):
                good_ind = ind
                if Species_tag_dict[good_ind] in refs_dict:
