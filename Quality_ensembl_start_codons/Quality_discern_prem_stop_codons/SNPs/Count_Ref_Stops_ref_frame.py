...
stop_codon_pos_ref = {}
for refer in refs_pos:
    positions = refs_pos[refer]
    for i in range(0, len(positions), 3):
        curr_seq = refs_dict[refer]
        ...
        if i < len(positions)-3:
            ...
            for record in alignment_obj:
                ...
                #print(miss)
            if any(cod in stop_codons for cod in reseq_codons) and \
            ref_codon not in stop_codons and "-" not in ref_codon:
