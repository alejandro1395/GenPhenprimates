...
OUTDIR=/scratch/devel/avalenzu/PhD_EvoGenom/GenomPhenom200primates/human_driven_results/Quality_start_codons/
mkdir -p ${OUTDIR}
SRC=/scratch/devel/avalenzu/PhD_EvoGenom/GenomPhenom200primates/src/Quality_start_codons/
...
python ${SRC}Find_best_start_codon.py ${INDIR}${gene_name}/${species_name}.within.cds.aln \
${gene_name} \
${species_name} \
${OUTDIR}${gene_name}/${species_name}.start_codons.txt
done
#tar arxives again
...
ls ${OUTDIR}${gene_name}/*.start_codons.txt | while read file; do cat $file | tail -n+2 >> ${OUTDIR}${gene_name}/All_start_codons.txt; done
rm ${OUTDIR}${gene_name}/*.start_codons.txt
