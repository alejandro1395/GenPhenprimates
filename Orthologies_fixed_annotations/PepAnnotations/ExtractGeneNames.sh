...
INDIR=/scratch/devel/avalenzu/PhD_EvoGenom/GenomPhenom200primates/data/Genomes/Annotations/REFS_FIXED/
...
for filepath in $(ls ${INDIR}*/*.sorted.gff);
do species_name=$(echo $filepath | rev | cut -d'/' -f1 | rev | cut -d \. -f 1)
cat $filepath | grep genename | cut -f 9 | tr "=" "\t" | tr ";" "\t" | cut -f 2,4 > ${INDIR}${species_name}/${species_name}.ref_genes.tsv
