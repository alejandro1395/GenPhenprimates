...
for spc in target_species:
    ...
    OrthoClust_df[spc].to_csv(out_file + spc + "/" + spc + ".pep.OrthoClustHumanprior_newBBHsLAX_low.gz", sep = "\t", index=False)
...
def print_final_output_for_clusters(last_clusters, ortho_DB, species_tag, out_file):
    for list_ortho in last_clusters:
        ...
        append_out_row_pandas_format(list_ortho,
                ortho_DB[final_spc], out_file + final_spc + "/" + final_spc + ".pep.OrthoClustHumanprior_newBBHsLAX_low.gz")
