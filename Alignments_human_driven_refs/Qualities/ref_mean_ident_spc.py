...
#READ TRIMAL FILES
alignments_count = 0
for file in os.listdir(trimals_path):
    ref_species = trimals_path.split("/")[-1]
    if file.endswith("SpeciesIdentities"):
        with open(trimals_path+"/"+file, 'r') as in_fh:
            species_count = 0
            alignments_count += 1
            ...
            for line in in_fh:
                ...
                if line.startswith("#Mean Percentage of identity:"):
                    ...
                    Species_mean_dict["Gene"].append(str(file).split(".")[1])
                    continue


#PRINT WHOLE VALUES FOR DISTRIBUTIONS
distributions = pd.DataFrame(Species_mean_dict)
distributions.to_csv(out_path+"/"+ref_species+".gene_distr_mean_ident_spc", sep = "\t", index=False)


#PRINT WHOLE VALUES FOR GENE DISTRIBUTIONS#create new df
distributions_gene = pd.DataFrame({"Genes_mean_ident":Genes_mean_ident})
distributions_gene.to_csv(out_path+"/"+ref_species+".gene_distr_mean_ident", sep = "\t", index=False)
