...
python ${SRC}Count_Ref_Stops_ref_frame.py ${INDIR}${gene_name}/${species_name}.within.cds.aln \
${gene_name} \
${species_name} \
${OUTDIR}${gene_name}/${species_name}.stop_codons_ref_frame.txt
done
#tar arxives again
...
ls ${OUTDIR}${gene_name}/*.stop_codons_ref_frame.txt | while read file; do cat $file | tail -n+2 >> ${OUTDIR}${gene_name}/All_stop_codons_ref_frame.txt; done
rm ${OUTDIR}${gene_name}/*.stop_codons_ref_frame.txt
