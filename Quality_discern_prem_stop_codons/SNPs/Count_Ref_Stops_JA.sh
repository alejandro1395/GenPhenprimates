...
${gene_name} \
${species_name} \
${OUTDIR}${gene_name}/${species_name}.stop_codons.txt
done
#tar arxives again
...
ls ${OUTDIR}${gene_name}/*.stop_codons.txt | while read file; do cat $file | tail -n+2 >> ${OUTDIR}${gene_name}/All_stop_codons.txt; done
rm ${OUTDIR}${gene_name}/*.stop_codons.txt
