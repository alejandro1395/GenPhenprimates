...
gunzip ${filepath};
cd-hit \
-i ${Primates}${species_name}/${species_name}.pep.fa \
-o ${OUTDIR}Query/${species_name}/${species_name}.nr100.pep.fa \
-c 1.0 \
-n 5 \
-M 16000 \
-d 0 \
-T 8;
gzip ${Primates}${species_name}/${species_name}.pep.fa;
gzip ${OUTDIR}Query/${species_name}/${species_name}.nr100.pep.fa" > ${OUTDIR}Query/qu/${species_name}.nr100.pep.sh
jobname=$(echo ${OUTDIR}Query/qu/${species_name}.nr100.pep.sh)
chmod 755 $jobname

#SUBMISSION TO CLUSTER
/scratch/devel/avalenzu/CNAG_interface/submit.py -c ${jobname} -o ${OUTDIR}Query/out/${species_name}.nr100.pep.out \
-e ${OUTDIR}Query/out/${species_name}.nr100.pep.err -n ${species_name} -u 1 -t 1 -w 00:05:00
done
