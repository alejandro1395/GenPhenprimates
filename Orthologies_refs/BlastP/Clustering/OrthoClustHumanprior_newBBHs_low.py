...
for spc in target_species:
    ...
    OrthoClust_df[spc].to_csv(out_file + spc + "/" + spc + ".pep.OrthoClustHumanprior_newBBH_low.gz", sep = "\t", index=False)
    ...
    for index, row in all_species_dfs.iterrows():
        ...
        if isNaN(row['Query']):
            ...
            if any(val in list(used_genes) for val in list(current_genes)):
                ...
                ortho_df[curr_species], max_key, out_file + curr_species + "/" + curr_species + ".pep.OrthoClustHumanprior_newBBH_low.gz")
