...
ortho_df, column_names, omas, used_genes):
    for query_species in all_species_dfs:
        ...
        for index, row in all_species_dfs.iterrows():
            cand_clusters = {}
            if isNaN(row['Query']) or row['Query'] in used_genes:
                continue
            else:
                ...
                for colname in column_names:
                    if isNaN(row[colname]) or row[colname] in used_genes:
                        continue
                    else:
                ...
                ortho_df[curr_species], max_key, out_file + curr_species + "/" + curr_species + ".pep.OrthoClustHumanprior.gz")
            ...
            for item in Trait_covs['SpeciesBroad']:
                if str(curr).startswith(item):
                    ...
                    if (num_traits > highest_num):
                        ...
                        highest_spcs = i
        final_spc = select_current_species(species_tag, highest_spcs)
    print(highest_spcs, final_spc, len(l[highest_spcs]))
