...
for filepath in $(ls ${QUERY}*/*.nr100.cds.fa.gz);
...
gunzip -c ${filepath} | blastn -query - \
-db ${TARGETDB}allspeciesDBnr_100.cds \
-evalue 1e-4 \
-num_alignments 0 \
-out ${OUTDIR}${species_name}/${species_name}.nr100.cds \
-num_threads 4;
gzip ${OUTDIR}${species_name}/${species_name}.nr100.cds" > ${OUTDIR}qu/${species_name}.nr100.cds.sh
jobname=$(echo ${OUTDIR}qu/${species_name}.nr100.cds.sh)
chmod 755 $jobname
