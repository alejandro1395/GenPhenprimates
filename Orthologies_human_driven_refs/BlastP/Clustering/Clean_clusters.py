...
first_clean_df = raw_df.dropna(thresh=2)
#Reorder based on species names as columns
for index, row in first_clean_df.iterrows():
    cluster_row = {}
    for value in row.values.tolist():
        if isNaN(value):
            ...
            cluster_row[curr_species] = value
    final_cleaned_df = final_cleaned_df.append(cluster_row, ignore_index = True)
