...
python ${SRC}OrthoClustHumanprior_newBBHsLAX_low.py ${INDIR} \
${SPECIES_IDs}summary_species.txt \
${TRAITS} \
${OUTDIR}" > ${OUTDIR}qu/all_species.pep.OrthoClustHumanprior_newBBHsLAX_low.sh
jobname=$(echo ${OUTDIR}qu/all_species.pep.OrthoClustHumanprior_newBBHsLAX_low.sh)
chmod 755 $jobname

#SUBMISSION TO CLUSTER
/scratch/devel/avalenzu/CNAG_interface/submit.py -c ${jobname} -o ${OUTDIR}out/all_species.pep.OrthoClustHumanprior_newBBHsLAX_low.out \
-e ${OUTDIR}out/all_species.pep.OrthoClustHumanprior_newBBHsLAX_low.err -n Clustering -u 1 -t 1 -w 1-23:59:00 -r lowprio
